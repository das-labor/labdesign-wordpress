...
?>
	<header>
		...
		<h2 class="entry-meta">
			...
			if ( $display_author )
				printf( __( 'von %s', 'tonic' ),
					'<a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '" title="' . esc_attr( sprintf( __( 'EintrÃge von %s', 'tonic' ), get_the_author() ) ) . '" rel="author">' . get_the_author() . '</a>'
				);
