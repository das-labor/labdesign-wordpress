...
function bavotasan_pagination() {
	...
	if ( ! empty( $pagination_return ) ) {
		...
		printf( __( 'Seite %1$s von %2$s', 'tonic' ), $current, $wp_query->max_num_pages );
 ...
 * @since 1.0.0
 */
function bavotasan_filter_wp_title( $title, $sep, $mode ) {
	global $paged, $page;
...
function bavotasan_excerpt_more( $text ) {
	$bavotasan_theme_options = bavotasan_theme_options();
	return $text . '<p class="more-link-p"><a class="btn btn-primary" href="' . get_permalink( get_the_ID() ) . '">' . __( 'Weiterlesen &rarr;' , 'tonic' ) . '</a></p>';
}
...
/*
 * Shortcode for events of the Labor calendar
 */

add_shortcode("laborcal", "LaborCal_ShortCode");

function LaborCal_ShortCode() {
	return file_get_contents('termine_html', TRUE);
}
